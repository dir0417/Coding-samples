...
process_file <- function(file_name) {
  ...
  df <- df %>% mutate('Event Started' = "2023-07-13 15:04", 'Event Ended' = "2023-07-13 16:04")
  
  # Add a filename column
  df <- df %>% mutate(FileName = basename(file_name))
  
  # Add a column to rename files based on filename, time, and date
  df <- df %>% mutate('Date Collected' = paste(FileName, format(Sys.time(), "%Y-%m-%d %H:%M:%S"), sep = "_"))
  df <- df %>% mutate('Timestamp' = paste(FileName, format(Sys.time(), "%H:%M:%S"), sep = "_"))
...
final_merged_remove_col_df <- final_merged_df %>%
  select('Facility Name','EMResource Region', 'Red Now', 'Yellow Now', 'Green Now', 'Last Update', 'Event Started', 
         'Event Ended', 'Date Collected', 'Timestamp', 'Address', 'County', 'Latitude', 'Longitude', 'License #  (State will populate)' )
